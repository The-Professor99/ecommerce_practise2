...
function Payments() {
  ...
  useEffect(() => {
    accountService
      .createPaymentIntent({ price: (getCartTotal(cart) * 100).toFixed(3) })
      .then((data) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const data = fakeCardConfirmPayment(clientSecret, {
      payment_method: {
          ...
          <section className="py-8 px-4 mb-8 bg-gray-200 rounded pr-4 pl-4 text-container">
            ...
            <div>
              <form id="payment-form" onSubmit={handleSubmit}>
                ...
                <p
                  className={
                    processed ? "result-message" : "result-message hidden"
                  }
                >
                  Cannot Process Payments now as the feature, which requires a
                  real backend, is still being built!.
                  <br />
